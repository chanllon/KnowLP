...
class KSSEnv(Env):
    def __init__(self, seed=None, initial_step=None):
        ...
        # 读取 JSON 文件
        graph_path = './data/junyi/prerequisite.json'
    ...
    def _initial_logs(self, learner: Learner):
        ...
        while len(logs) < self._initial_step:
            if random.random() < 0.7:
                ...
                # learning_item_id = self.learning_item_base.index[str(int(random.choice(list(self.knowledge_structure.nodes()))))].id
                learning_item_id = int(random.choice(list(self.knowledge_structure.nodes())))
    ...
    def learn_and_test(self, learner: Learner, item_id):
        learning_item = self.learning_item_base[str(item_id)]
    ...
    def _exam(self, learner: Learner, detailed=False, reduce=None) -> (dict, int, float):
        ...
        for test_knowledge in learner.target:
            if test_knowledge == 0: test_knowledge = 1
            if test_knowledge == 442 : test_knowledge = 441
            item = self.test_item_base.knowledge2item[test_knowledge][0]
    ...
    def begin_episode(self, *args, **kwargs):
        self.learner = next(self.learners)
        self._initial_logs(self.learner)
        self._initial_score = self._exam(self.learner)
        return self.learner.profile, self._exam(self.learner, detailed=True)
    ...
    def step(self, ques_H, ans_H, learning_item_id, *args, **kwargs):
        ...
        state = state[:, -1, :]
        return state, observation, float(b - a)/len(self.learner.target), b == len(self.learner.target), ques_H, ans_H
    ...
    def step_p(self, ques_H, ans_H, practice_item_id, *args, **kwargs):
        observation = self.learn_and_test(self.learner, practice_item_id)
        ...
        ques = deepcopy(ques_H)
        ques.append(int(observation[0]))
        ans = deepcopy(ans_H)
        ans.append(int(observation[1]))
        kc = self.learning_item_base.index[str(practice_item_id)].knowledge
        state = self.KT_model.forward_state(ques,ans)
        state = state[:, -1, :]
        know_state = self.retrive_state(state, self.know_item)
        return state, observation, -math.pow((cur_diff - last_diff), 5), (float(state.squeeze(0)[kc]) > self.mastery_thresh_hold), ques, ans, cur_diff
    ...
    def step_fordata(self, practice_item_id, *args, **kwargs):
        ...
        observation = self.learn_and_test(self.learner, practice_item_id)
        b = self._exam(self.learner)
        return observation, float(b - a)/len(self.learner.target), b == len(self.learner.target)
    ...
    def render(self, mode='human'):
        if mode == "log":
            return "target: %s, state: %s" % (
                self.learner.target, dict(self._exam(self.learner))
            )
