...
import rl_utils
import random
from AdaptiveCN import Adaptive_Cognitive_Nevigation, Find_Longest_Path_With_Threshold
import collections
...
class PPO_Pre:
    ...
    def take_action(self, state, last_item, target, Know_G,k_hop,threshold, last_tor=3,last_prac_num=2, explore=False):
        if last_item == None:
            last_item, _ = Find_Longest_Path_With_Threshold(Know_G, target, state, threshold)
        probs = self.actor(state.to(device))
        mastery = float(state[0][int(last_item)])
        candidates = Adaptive_Cognitive_Nevigation(target, Know_G, last_item, mastery>threshold, k_hop)
        candidates_probs = []
